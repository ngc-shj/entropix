...
```
poetry run python download_weights.py --model-id meta-llama/Llama-3.1-8B --out-dir weights/8B-Base
poetry run python download_weights.py --model-id meta-llama/Llama-3.1-8B-Instruct --out-dir weights/8B-Instruct
```
...
run it (torch)
```bash
 PYTHONPATH=. poetry run python entropix/torch_main.py --model-size 8B
```
